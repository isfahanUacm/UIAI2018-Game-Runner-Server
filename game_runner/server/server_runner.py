...
def start(team1_name, team2_name, log_file_name=datetime.now().isoformat()):
    # --mode=cvc --logfile=log_file_name.log --speed=20 --team1name=chom --team2name=choooom -nographic -batchmode
    command = ['./{}'.format(SERVER_FILE_NAME),
               '--mode=cvc', '--logfile={}.log'.format(log_file_name), '--speed=20', '-nographic', '-batchmode',
